...
"""
================================================================================
domain_adaptation/methods/run_anchored_lora.py
Trial #10: PI-LoRA with Pretrained Head Anchoring & Class-Preserving Regularization.
Solves the "Catastrophic Forgetting of Absent Classes" in 10-Fold LODO (e.g. Step_R & Step_T).
================================================================================
"""
...
def run_anchored_lora_experiment(
    model_dir: str = None,
    output_dir: str = None,
    epochs: int = 70,
    rank: int = 4,
    ...
    method_name = "PI_LoRA_Anchored"
    print(f"\n[START] {method_name} | Rank={rank}, Alpha={alpha}, AnchorWeight={anchor_weight}, CosineHead={use_cosine_head}")
    ...
    if output_dir is None:
        output_dir = os.path.join(PROJECT_ROOT, "domain_adaptation", "results", "trial_10_pi_lora_anchored", tag)
    os.makedirs(output_dir, exist_ok=True)

    # 2. Load & Calibrate Real Data
    ...
    for fold_id, fold_info in folds.items():
        ...
        # Clone clean base model and inject LoRA
        model = copy.deepcopy(base_model).to(device)
        model = inject_lora_into_model(model, rank=rank, alpha=alpha).to(device)

        # Store initial pretrained classifier weights for anchoring
        orig_clf_params = {name: p.clone().detach() for name, p in model.classifier.named_parameters()}
        orig_final_weight = model.classifier[-1].weight.clone().detach()
        ...
        train_shape_indices = [shape_to_idx.get(metadata_list[i]['true_shape'], 0) for i in train_idx]
        present_classes = set(train_shape_indices)
        absent_classes = [c for c in range(len(UNIQUE_SHAPES)) if c not in present_classes]

        y_shape_target = torch.tensor(train_shape_indices, dtype=torch.long, device=device)
        ...
        for _ in range(epochs):
            ...
            # Elastic Head Anchoring Loss (penalize drift from pretrained simulation weights)
            loss_anchor = sum(F.mse_loss(p, orig_clf_params[n]) for n, p in model.classifier.named_parameters())

            # Strongly preserve absent class prototype directions in the final layer
            if absent_classes:
                absent_idx_tensor = torch.tensor(absent_classes, dtype=torch.long, device=device)
                loss_absent = F.mse_loss(
                    model.classifier[-1].weight[absent_idx_tensor],
                    orig_final_weight[absent_idx_tensor]
                )
                loss_anchor = loss_anchor + 2.0 * loss_absent

            total_loss = loss_task + phys_weight * loss_vol + anchor_weight * loss_anchor
    ...
    print("\n" + "=" * 80)
    print(f"{method_name.upper()} SUMMARY ({tag})")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    run_anchored_lora_experiment(
        ...
        use_cosine_head=args.cosine_head,
