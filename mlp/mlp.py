...
def _log_uncaught_exception(exc_type, exc_value, exc_tb):
    script_name = os.path.basename(__file__) if '__file__' in globals() else 'mlp.py'
...
OUTPUT_ROOT_DIR = os.environ.get(
    "PINN_OUTPUT_ROOT", os.path.join(SCRIPT_DIR, "Outputs_mlp_pinn")
)
...
if ALPHA_INIT == 0:
    print("[WARN] ALPHA_INIT=0: physics loss term disabled (running in no-PINN mode)")
...
def save_confusion_matrix(y_true, y_pred, unique_shapes, output_dir, filename="confusion_matrix.png", title="Confusion Matrix"):
    """Saves a confusion matrix to a specific file."""
    _set_ieee_style()
    try:
        cm = confusion_matrix(y_true, y_pred)
    except Exception:
    ...
    try:
        ...
        with torch.no_grad():
            for batch_X, _, _ in test_loader:
                ...
                # Intermediate regression features if reg_head exists
                if hasattr(model, 'reg_head') and len(model.reg_head) >= 3:
                    r_feat = model.reg_head[:3](b_feat)
                else:
                    r_feat = b_feat
        ...
        for fkey, flabel, raw_f in [
            ("backbone", "Shared MLP Backbone", backbone_feats),
        ...
        fig.suptitle(f"MLP t-SNE Latent Space Representation{t_suf}", fontsize=19, fontweight="bold", y=0.99)
        fig.tight_layout()
        for ext in (".png", ".pdf"):
        ...
        fig.suptitle(f"MLP Physics Geometry Disentanglement on t-SNE Manifold{t_suf}", fontsize=19, fontweight="bold", y=0.99)
        fig.tight_layout()
        for ext in (".png", ".pdf"):
...
def compute_combined_loss(clf_loss, w_loss, l_loss, d_loss, physics_loss,
                         alpha=1.0):
    """
    Compute combined loss used by the active training path.

    Formula:
        total = clf + ((w + l + d)/3) + alpha * physics
    ...
    total_loss = clf_loss + reg_loss + alpha * physics_loss
...
# Keep old name as alias for backward compatibility (checkpoint loading in run_eval_all.py)
ImprovedMultimodelNet = None  # Will be set after class definition

class MultitaskMLP_PINN(nn.Module):
    """
    MLP-based multitask network for shape classification + joint W/L/D regression.

    Adapted from Xiong et al. 2023 architecture:
    ...
    - Direct Regression head: 16 -> 3 (W, L, D) with Softplus activation (strictly positive)
    - Pure MLP formulation without BatchNorm/Dropout matching Table 3
    """
    INPUT_DIM = 32 * 32 * 2  # 2048

    def __init__(self, num_shapes, input_dim=None):
    ...
    def forward(self, x):
        """Forward pass. x shape: (N, 2048) — already flattened."""
        if x.dim() > 2:
            x = x.view(x.size(0), -1)
...
# Backward-compatible alias so existing checkpoint loaders still resolve the name.
...
print(f"\nBUILDING MULTITASK MLP-PINN MODEL (Xiong et al. 2023)")

num_shapes = len(unique_shapes)
model = MultitaskMLP_PINN(num_shapes).to(device)
...
def save_checkpoint_plots(history, epoch, checkpoint_dir, unique_shapes=None,
    ...
    if y_test is not None and y_pred_wld is not None:
        ...
        metrics_text += f"Accuracy:     {clf_acc_text}\n"
...
print(f'  Warm-up (epochs 1-{PINN_ACTIVATION_EPOCH}): L = L_clf + L_reg (PINN inactive)')
print(f'  PINN phase (epoch {PINN_ACTIVATION_EPOCH + 1}+): L = L_clf + L_reg + alpha*L_pinn (single optimizer step per batch)')
...
for epoch in range(start_epoch, EPOCHS):
    ...
    for batch_idx, (batch_X, batch_y_wld, batch_y_shape) in enumerate(pbar):
        ...
        with autocast('cuda', enabled=amp_enabled):
            shape_logits, y_pred_wld = model(batch_X.float() if pinn_loss_active else batch_X)
            ...
            # Compute data losses: average regression loss across W, L, D, then balance with classification
            ...
            data_loss = (
                precision_clf * clf_loss + 0.5 * model.log_var_clf +
                precision_reg * avg_reg_loss + 0.5 * model.log_var_reg
            )
        ...
        if is_lbfgs_phase:
            def closure():
                ...
                s_logits_c, y_pred_c = model(batch_X.float())
                clf_l_c = criterion_clf(s_logits_c, batch_y_shape)
                _, _, _, reg_l_c = compute_regression_losses(y_pred_c, batch_y_wld, criterion_reg)
                p_clf_c = torch.exp(-model.log_var_clf)
                p_reg_c = torch.exp(-model.log_var_reg)
                d_loss_c = p_clf_c * clf_l_c + 0.5 * model.log_var_clf + p_reg_c * reg_l_c + 0.5 * model.log_var_reg
                phys_l_c = torch.tensor(0.0, device=device)
                if pinn_loss_active:
                    c_losses = []
                    for idx_c in range(batch_X.size(0)):
                        f_val_c = batch_X[idx_c].view(32, 32, 2)[:, :, 0]
                        s_idx_c = int(batch_y_shape[idx_c].item())
                        s_name_c = unique_shapes[s_idx_c]
                        if s_name_c in SHAPE_MAP_TRAINING:
                            try:
                                p_loss = compute_physics_loss_autograd(
                                    f_val_c, y_pred_c[idx_c, 0], y_pred_c[idx_c, 1], y_pred_c[idx_c, 2],
                                    s_name_c, SHAPE_MAP_TRAINING, y_scaler, X_scaler=X_scaler,
                                    w_true=batch_y_wld[idx_c, 0], l_true=batch_y_wld[idx_c, 1], d_true=batch_y_wld[idx_c, 2]
                                )
                                c_losses.append(p_loss)
                            except Exception:
                                pass
                    if c_losses:
                        phys_l_c = torch.stack(c_losses).mean()
                loss_c = d_loss_c + alpha * phys_l_c
                loss_c.backward()
                return loss_c
...
if nan_detected:
    print("\n" + "="*80)
    print(f"[CRITICAL][NAN DETECTED] MLP training stopped early due to NaN detection!")
...
save_confusion_matrix(y_shape_test, y_pred_shape, unique_shapes, OUTPUT_DIR, "test_confusion_matrix.png", title="Final Test Confusion Matrix")

    # Regression metrics (using DENORMALIZED values)
print(f"\n--- REGRESSION (Denormalized Values) ---")
metrics_names = ['Width (W)', 'Length (L)', 'Depth (D)']
...
balanced_accuracy_val = float(balanced_accuracy_score(y_shape_test, y_pred_shape))
mcc_val = float(matthews_corrcoef(y_shape_test, y_pred_shape))
precision_score_val = float(precision_score(y_shape_test, y_pred_shape, average='macro', zero_division=0))
recall_score_val = float(recall_score(y_shape_test, y_pred_shape, average='macro', zero_division=0))
f1_score_val = float(f1_score(y_shape_test, y_pred_shape, average='macro', zero_division=0))

class_metrics_df = _build_class_metrics_df(
    np.array(y_shape_test),
    np.array(y_pred_shape),
    unique_shapes,
    TRAIN_PERCENT,
    pinn_alpha=ALPHA_INIT if physics_available_for_training else 0.0,
    pinn_active=physics_available_for_training,
)
class_metrics_csv = os.path.join(OUTPUT_DIR, 'test_class_metrics.csv')
...
print(f"\n{'='*80}")
print("PER-SHAPE DETAILED ANALYSIS (Q1 STANDARDS)")
print(f"{'='*80}")
...
# Visualize per-shape metrics comparison
if per_shape_results:
    try:
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.suptitle(f'Per-Shape Metrics Comparison (Train {TRAIN_PERCENT}%)', fontsize=14, fontweight='bold')
        ...
        # MAE per shape (avg)
        ax = axes[0, 1]
        maes = [r['mae_avg'] for r in per_shape_results]
        ...
        # RMSE per shape (avg)
        ax = axes[1, 0]
        rmses = [r['rmse_avg'] for r in per_shape_results]
        ...
        plt.tight_layout()
        per_shape_plot = os.path.join(OUTPUT_DIR, 'test_metrics_per_shape.png')
