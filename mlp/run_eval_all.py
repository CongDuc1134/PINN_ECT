...
if EVAL_PRESET == "all_existing":
    ...
    os.environ["ALPHA_TEST_VALUES"] = "0,1,10,100,1000"
    os.environ["ALPHA_TEST_WARMUP"] = "0"
    os.environ["ALPHA_TEST_PERCENTS"] = "5"
    os.environ["ALPHA_TEST_SEEDS"] = "42"
    os.environ["BASELINE_ALPHA"] = "1"
    os.environ["BASELINE_WARMUP"] = "0"
    os.environ["BASELINE_PERCENTS"] = "1,3,5,7,10"
    os.environ["BASELINE_SEEDS"] = "42"
    os.environ["SEED_TEST_ALPHA"] = "1"
    os.environ["SEED_TEST_WARMUP"] = "0"
...
PINN_ACTIVATION_EPOCH = _get_int_env("PINN_ACTIVATION_EPOCH", 0, min_value=0)
SEEDS = [s.strip() for s in os.environ.get("SEEDS", "42,123,456,789").split(',') if s.strip()]
if not SEEDS:
    SEEDS = ["42"]
...
ALPHA_VALUES = _parse_alpha_values(
    os.environ.get("ALPHA_VALUES", "1.0")
)
# Remove alpha==0 since that's equivalent to No-PINN
if 0 in ALPHA_VALUES:
    ALPHA_VALUES = [a for a in ALPHA_VALUES if a != 0]
WARMUP_EPOCHS_RAW = os.environ.get("WARMUP_EPOCHS", DEFAULT_WARMUP_EPOCHS)
WARMUP_EPOCHS = []
for chunk in WARMUP_EPOCHS_RAW.split(','):
    t = chunk.strip()
    if not t:
        continue
    try:
        WARMUP_EPOCHS.append(int(t))
    except Exception:
        raise ValueError(f"Invalid WARMUP_EPOCHS entry: {t}")
...
SCRIPT_NO_PINN = os.path.join(SCRIPT_DIR, "mlp.py")
SCRIPT_PINN = os.path.join(SCRIPT_DIR, "mlp.py")
...
OUTPUT_ROOT = os.path.abspath(
    os.environ.get("EVAL_OUTPUT_ROOT", os.path.join(SCRIPT_DIR, "Outputs_mlp_eval"))
)
STUDY_ROOT = os.path.join(OUTPUT_ROOT, STUDY_NAME)
PINN_MODEL_ROOT = os.path.join(SCRIPT_DIR, "Outputs_mlp_pinn")
NO_PINN_MODEL_ROOT = os.path.join(SCRIPT_DIR, "Outputs_mlp_baseline")
...
def collapse_equivalent_plot_series(series_items):
    ...
    for item in series_items:
        ...
        existing["label"] = f"{existing['_plot_alias_labels'][0]} (+{len(existing['_plot_alias_labels']) - 1} same-config)"
    return collapsed
...
def choose_best_alpha_from_alpha_series(series_items, percent_priority):
    """Choose the alpha with lowest mean NRMSE (or RMSE) over the same percent set."""
    ...
    for item in series_items:
        ...
        for pct in pcts:
            try:
                # Prioritize nrmse, fallback to rmse, then mae
                val = float(results[pct].get("nrmse", results[pct].get("rmse", results[pct].get("mae", np.nan))))
                if np.isfinite(val):
                    vals.append(val)
                else:
                    missing.append(pct)
    ...
    if not candidates:
        if excluded:
            print("[AUTO][WARN] No alpha has complete NRMSE/RMSE coverage for best-alpha selection.")
    ...
    best_val, best_alpha, best_key, best_pcts = candidates[0]
    print(
        f"[AUTO] Best alpha from alpha sweep: alpha={alpha_to_label(best_alpha)} "
        f"(mean NRMSE/RMSE={best_val:.6g}, source={best_key}, percents={best_pcts})"
    )
    return best_alpha
...
def _build_reeval_model_class(torch, nn):
    class MultitaskMLP_PINN(nn.Module):
        ...
        def __init__(self, num_shapes):
            ...
            # ===== SHARED MLP BACKBONE (3 HIDDEN LAYERS) [XIONG ET AL. 2023 TABLE 3] =====
            ...
            self.regressor_backbone = nn.Identity()
...
def save_single_model_feature_visualizations(
    ...
    for feature_key, feature_label, raw_features in [
        ("backbone", "Shared MLP Backbone", backbone_features),
...
def _reeval_prepare_split(percent, seed):
    ...
    X_test_tensor = torch.FloatTensor(X_test_norm.reshape(n_test, -1))
...
def _reeval_infer_existing_model(run_dir, mode, percent, seed, item):
    ...
    with torch.no_grad():
        for batch_X, _, _ in split["loader"]:
            ...
            regression_feature_parts.append(backbone_features.cpu().numpy())
    ...
    result_row = {
        **common_meta,
        "clf_acc": clf_acc,
        "clf_balanced_acc": clf_metrics["balanced_accuracy"],
        "clf_f1_macro": clf_metrics["f1_macro"],
        "clf_mcc": clf_metrics["mcc"],
        **{f"clf_{key}": value for key, value in clf_metrics.items() if key != "accuracy"},
        "mae_avg": float(np.mean(metrics["mae"])),
        "rmse_avg": float(np.mean(metrics["rmse"])),
        "r2_avg": float(np.mean(metrics["r2"])),
        "nrmse_avg": float(np.nanmean(metrics["nrmse"])),
        "max_error_avg": float(np.mean(metrics["max_error"])),
    ...
    for cls_idx, class_name in enumerate(unique_shapes):
        ...
        for target_idx in range(3):
            ...
            rmse = float(math.sqrt(mse))
            true_r = float(np.max(y_true[mask, target_idx]) - np.min(y_true[mask, target_idx]))
            nrmse = float(rmse / true_r * 100.0) if true_r > 1e-12 else float("nan")
            ...
            shape_max_err.append(float(max_error(y_true[mask, target_idx], y_pred[mask, target_idx])))
            shape_nrmse.append(nrmse)
    ...
    print(
        ...
        f"acc={clf_acc*100.0:.2f}% nrmse={result_row['nrmse_avg']:.2f}% rmse={result_row['rmse_avg']:.4f}"
    )
    return result_row
...
def _missing_metric_groups(run_dir, item, percent, seed):
    ...
    regression_target = {
        ...
        "nrmse", "error_mean", "error_std",
        "true_mean", "pred_mean", "n_samples",
    }
    regression_overall = {
        f"{metric}_avg"
        for metric in (
            "mae", "rmse", "r2", "nrmse", "max_error",
        )
    }
    regression_shape = {
        f"{metric}_{suffix}"
        ...
        for suffix in ("w", "l", "d")
    } | {"mae_avg", "rmse_avg", "r2_avg", "nrmse_avg", "max_error_avg", "bias_avg", "clf_acc", "n_samples"}
...
def save_full_metric_tables(metrics_long_df, report_dir):
    ...
    key_metrics = {
        ("classification_overall", "overall"): [
            "clf_accuracy", "clf_balanced_accuracy", "clf_precision_macro",
            "clf_recall_macro", "clf_f1_macro", "clf_mcc",
        ],
        ("regression_overall", "average_wld"): [
            "mae_avg", "rmse_avg", "r2_avg", "nrmse_avg", "max_error_avg", "mean_error_avg",
        ],
    }
    overview_parts = []
    ...
    saved_paths = [overview_csv]

    def render_table(raw_table, direction_by_column, title, path):
        raw_table = raw_table.dropna(axis=0, how="all")
    ...
    for scope in sorted(df["scope"].dropna().unique()):
        ...
        ordered_columns = (
            scope_df.groupby("column_name")["value"]
            .count()
            .sort_values(ascending=False)
            .index.tolist()
        )
        for chunk_idx, col_start in enumerate(range(0, len(ordered_columns), max_columns), start=1):
            cols_chunk = ordered_columns[col_start : col_start + max_columns]
            chunk_df = scope_df[scope_df["column_name"].isin(cols_chunk)].copy()
            ...
            direction_map = (
                chunk_df.drop_duplicates("column_name")
                .set_index("column_name")["better_direction"]
                .to_dict()
            )
            part_suffix = f"_part{chunk_idx}" if len(ordered_columns) > max_columns else ""
            table_title = f"{scope.replace('_', ' ').title()} - {scope.replace('_', ' ')} metrics{part_suffix}"
            out_file = os.path.join(output_dir, f"{scope}{part_suffix}.png")
            render_table(pivot, direction_map, table_title, out_file)

    print(f"[OK] Saved full metric tables: {output_dir}")
    return saved_paths


def save_all_metrics_long_csv(series_items, report_dir):
    """Collect every evaluation artifact and export a single normalized long-form CSV."""
    rows = []
    seen = set()

    def metric_metadata(scope, metric):
        if metric in {"nrmse", "nrmse_range", "nrmse_mean"} or metric.startswith("nrmse_"):
            return "NRMSE (%)", "lower", "%"
        if metric in {"mean_error", "error_mean"} or metric.startswith(("mean_error_", "bias_")):
            return "Bias (mm)" if metric in {"mean_error", "error_mean"} else metric.replace("mean_error", "bias"), "closer_to_zero", "mm"
        if metric in {"mae", "mse", "rmse", "median_ae", "max_error"} or metric.startswith(
            ("mae_", "mse_", "rmse_", "median_ae_", "max_error_")
        ):
            unit = "mm²" if "mse" in metric else "mm"
        ...
        if metric in {"mcc", "clf_mcc", "mcc_ovr"}:
            return "MCC", "higher", ""
        if metric in {"balanced_accuracy", "clf_balanced_accuracy", "balanced_accuracy_ovr"}:
            return "Balanced Accuracy", "higher", "%"
        if metric in {"f1_macro", "clf_f1_macro"}:
            return "F1-Macro", "higher", "%"
        if metric in {"fp", "fn"}:
        ...
        if scope.startswith("classification"):
            unit = "%" if metric.endswith("_percent") else ""
        ...
        for seed in seed_list:
            ...
            if reuse_completed and has_completed_seed_result(mode, pct, seed_tag):
                ...
                if progress_callback is not None:
                    try:
                        progress_callback(reason=f"skip seed={seed} pct={pct} tag={custom_tag}")
                    except Exception as exc:
    ...
    for pct_folder in os.listdir(folder_root):
        ...
        for run_folder in run_folders:
            ...
            try:
                ...
                if len(df) > 0:
                    row = df.iloc[0]
                    clf_acc_raw = float(df.iloc[0]["clf_acc"]) if "clf_acc" in df.columns else (float(df.iloc[0]["acc"]) if "acc" in df.columns else np.nan)
                    clf_acc_pct = clf_acc_raw * 100.0 if np.isfinite(clf_acc_raw) and clf_acc_raw <= 1.0 else clf_acc_raw
                    ...
                    nrmse_val = float(df.iloc[0]["nrmse_avg"]) if "nrmse_avg" in df.columns else (float(df.iloc[0]["nrmse"]) if "nrmse" in df.columns else np.nan)
                    ...
                    balanced_acc_val = float(df.iloc[0]["clf_balanced_acc"]) if "clf_balanced_acc" in df.columns else (float(df.iloc[0]["balanced_acc"]) if "balanced_acc" in df.columns else np.nan)
                    if np.isfinite(balanced_acc_val) and balanced_acc_val <= 1.0:
                        balanced_acc_val = balanced_acc_val * 100.0
                    ...
                    for col, val in row.items():
                        if col in {"clf_acc", "mae_avg", "rmse_avg", "r2_avg", "nrmse_avg", "max_error_avg", "clf_balanced_acc", "clf_f1_macro", "clf_mcc"}:
                            continue
                        try:
                            rec[col] = float(val)
                        except Exception:
    ...
    for s_idx, item in enumerate(plot_series):
        ...
        for p in tgt_pcts:
            ...
            if not np.isfinite(val):
                if metric_key == "mae":
                ...
                elif metric_key == "nrmse":
                    val = info.get("nrmse_avg", np.nan)
                    ...
                    if np.isfinite(val) and val <= 1.0:
                        val = val * 100.0
                elif metric_key == "balanced_acc":
                    val = info.get("clf_balanced_acc", np.nan)
                    if np.isfinite(val) and val <= 1.0:
                        val = val * 100.0
                elif metric_key == "f1_macro":
                    val = info.get("clf_f1_macro", np.nan)
                    if np.isfinite(val) and val <= 1.0:
                        val = val * 100.0
                elif metric_key == "mcc":
                    val = info.get("clf_mcc", np.nan)
            values.append(val)
        ...
        for p, v in valid_pts:
            if metric_key in ("acc", "balanced_acc", "f1_macro", "nrmse") or "%" in y_label:
                ...
                val_text = f"{v:.4f}" if abs(v) < 1 else f"{v:.3f}"
            txt = ax.annotate(
                val_text,
                (p, v),
...
def save_percent_bar_plots(tgt_pcts, series_dict, report_dir):
    ...
    for pct in tgt_pcts:
        ...
        for item in series_dict:
            ...
            rmse_vals.append(res.get("rmse", np.nan))
            mae_vals.append(res.get("mae", np.nan))
            nrmse_vals.append(res.get("nrmse", np.nan))
            r2_vals.append(res.get("r2", np.nan))
            acc_vals.append(res.get("acc", np.nan))
            f1_vals.append(res.get("f1_macro", np.nan))
            mcc_vals.append(res.get("mcc", np.nan))
            
            row = {
                "train_percent": pct,
                "series": item["label"],
                "rmse_avg": res.get("rmse", np.nan),
                "mae_avg": res.get("mae", np.nan),
                "nrmse_avg": res.get("nrmse", np.nan),
                "r2_avg": res.get("r2", np.nan),
                "clf_acc_percent": res.get("acc", np.nan),
                "clf_f1_macro": res.get("f1_macro", np.nan),
                "clf_mcc": res.get("mcc", np.nan),
        ...
        # Helper to plot a bar chart
        def _plot_single_bar(values, title, ylabel, filename, is_pct=False, fmt=".4f"):
            ...
            try:
                ...
                ax.set_xticks(x)
                ax.set_xticklabels(labels, rotation=22, ha="right", fontsize=10, fontweight="bold")
                ax.set_title(f"{title} - Train {pct}%", fontweight="bold", fontsize=13)
                ax.set_ylabel(ylabel, fontweight="bold", fontsize=11)
                ax.grid(axis="y", linestyle="--", alpha=0.45)
                for bar, val in zip(bars, values):
                    if np.isfinite(val):
                        val_str = f"{val:.2f}%" if is_pct else f"{val:{fmt}}"
                        txt = ax.annotate(
                            val_str,
                ...
                fig.tight_layout()
                out_path = os.path.join(percent_dir, filename)
                fig.savefig(out_path, dpi=220, bbox_inches="tight")
            except Exception as e:
                print(f"[WARN] Failed saving {filename}: {e}")
            finally:
                plt.close(fig)

        _plot_single_bar(rmse_vals, "RMSE by Configuration", "RMSE (mm) -> Lower is better", "rmse_bar.png", fmt=".4f")
        _plot_single_bar(mae_vals, "MAE by Configuration", "MAE (mm) -> Lower is better", "mae_bar.png", fmt=".4f")
        _plot_single_bar(nrmse_vals, "NRMSE by Configuration", "NRMSE (%) -> Lower is better", "nrmse_bar.png", is_pct=True)
        _plot_single_bar(r2_vals, "R² Score by Configuration", "R² Score -> Higher is better", "r2_bar.png", fmt=".4f")
        _plot_single_bar(acc_vals, "Accuracy by Configuration", "Accuracy (%) -> Higher is better", "acc_bar.png", is_pct=True)
        _plot_single_bar(f1_vals, "F1-Macro by Configuration", "F1-Macro (%) -> Higher is better", "f1_macro_bar.png", is_pct=True)
        _plot_single_bar(mcc_vals, "MCC by Configuration", "Matthews Correlation Coefficient -> Higher is better", "mcc_bar.png", fmt=".4f")
...
def save_warmup_metric_bar_plots(warmup_series, report_dir):
    ...
    for pct in percent_values:
        ...
        for metric_key, metric_label, higher_is_better in [
            ...
            avg_key = f"{metric_key}_avg" if f"{metric_key}_avg" in records[0] else metric_key
            avg_values = np.asarray(
                [float(record.get(avg_key, record.get(f"{metric_key}_avg", np.nan))) for record in records],
                dtype=float,
            )
...
def save_seed_grouped_bar_plots(tgt_pcts, seed_series, report_dir, *, live=False):
    """Save paired No-PINN/PINN bars for each seed at each selected percent."""
    ...
    for pct in tgt_pcts:
        ...
        for item in seed_series:
            ...
            if len(seed_list) != 1:
                continue
            seed = str(seed_list[0])
            rec = item.get("results", {}).get(pct)
            if rec is None:
                continue
            mode = item.get("mode")
            by_seed[seed][mode] = rec
            rows.append({
                "train_percent": pct,
                "seed": seed,
                "mode": mode,
                "series": item.get("label", ""),
                "rmse_avg": rec.get("rmse", np.nan),
                "mae_avg": rec.get("mae", np.nan),
                "nrmse_avg": rec.get("nrmse", np.nan),
                "r2_avg": rec.get("r2", np.nan),
                "clf_acc_percent": rec.get("acc", np.nan),
                "clf_f1_macro": rec.get("f1_macro", np.nan),
                "clf_mcc": rec.get("mcc", np.nan),
        ...
        _atomic_to_csv(pd.DataFrame(rows), table_path, index=False)
        ...
        plot_specs = [
            (
                ...
                f"seed_rmse_grouped_bar_train_{pct:02d}pct.png",
                False,
            ),
            (
                "mae",
                ...
                f"seed_mae_grouped_bar_train_{pct:02d}pct.png",
                False,
            ),
            (
                "nrmse",
                ...
                f"seed_nrmse_grouped_bar_train_{pct:02d}pct.png",
                True,
            ),
            (
                "r2",
                ...
                f"seed_r2_grouped_bar_train_{pct:02d}pct.png",
                False,
            ),
            (
                "acc",
                ...
                f"seed_acc_grouped_bar_train_{pct:02d}pct.png",
                True,
            ),
            (
                "f1_macro",
                "Seed Test F1-Macro",
                "F1-Macro (%) - Higher is better",
                f"seed_f1_macro_grouped_bar_train_{pct:02d}pct.png",
                True,
            ),
            (
                "mcc",
                ...
                f"seed_mcc_grouped_bar_train_{pct:02d}pct.png",
                False,
            ),
        ]

        for metric_key, title, y_label, filename, is_pct in plot_specs:
            fig, ax = plt.subplots(figsize=(10, 6))
            plotted = False
            for mode_idx, (mode, mode_label, color) in enumerate(mode_specs):
                offset = (mode_idx - 0.5) * width
                values = [
                    float(by_seed[seed].get(mode, {}).get(metric_key, np.nan))
                    for seed in seeds
                ]
                bars = ax.bar(
                    x + offset,
                ...
                for bar, value in zip(bars, values):
                    if not np.isfinite(value):
                        continue
                    plotted = True
                    val_str = f"{value:.2f}%" if is_pct else f"{value:.4f}"
                    ax.annotate(
                        val_str,
                        xy=(bar.get_x() + bar.get_width() / 2.0, bar.get_height()),
                        xytext=(0, 3),
            ...
            ax.set_title(f"{prefix}{title} - Train {pct}%", fontsize=14, fontweight="bold")
    ...
    print(f"[OK] Saved separate seed grouped bar charts to: {report_dir}")
...
def refresh_live_comparison_plots(series_dict, report_dir):
    ...
    main_series = [s for s in series_dict if s.get("group") != "seed"]
    ...
    if main_series:
        ...
        if main_percents:
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="mae",
                title="LIVE Average Output Test MAE",
                y_label="Average Test MAE (mm) $\\downarrow$",
                out_file=os.path.join(main_dir, "comparison_mae_live.png"),
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="rmse",
                title="LIVE Average Output Test RMSE",
                y_label="Average Test RMSE (mm) $\\downarrow$",
                out_file=os.path.join(main_dir, "comparison_rmse_live.png"),
                lower_is_better=True,
            )
            save_line_plot(
                main_percents,
                main_series,
                metric_key="r2",
                title="LIVE Average Output Test R² Score",
                y_label="Average Test R² Score $\\uparrow$",
                out_file=os.path.join(main_dir, "comparison_r2_live.png"),
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="nrmse",
                title="LIVE Average Output Test NRMSE",
                y_label="Average Test NRMSE (%) $\\downarrow$",
                out_file=os.path.join(main_dir, "comparison_nrmse_live.png"),
                lower_is_better=True,
            )
            save_line_plot(
                main_percents,
                ...
                title="LIVE Output Test Max Error",
                y_label="Max Error (mm) $\\downarrow$",
                out_file=os.path.join(main_dir, "comparison_max_error_live.png"),
                lower_is_better=True,
            )
            save_line_plot(
                main_percents,
                main_series,
                metric_key="acc",
                title="LIVE Classification Accuracy",
                y_label="Classification Accuracy (%) $\\uparrow$",
                out_file=os.path.join(main_dir, "comparison_acc_live.png"),
                lower_is_better=False,
            )
            save_line_plot(
                main_percents,
                ...
                title="LIVE Balanced Classification Accuracy",
                y_label="Balanced Accuracy (%) $\\uparrow$",
                out_file=os.path.join(main_dir, "comparison_balanced_acc_live.png"),
                lower_is_better=False,
            )
            save_line_plot(
                main_percents,
                main_series,
                metric_key="f1_macro",
                title="LIVE Classification F1-Macro",
                y_label="F1-Macro (%) $\\uparrow$",
                out_file=os.path.join(main_dir, "comparison_f1_macro_live.png"),
                lower_is_better=False,
            )
            save_line_plot(
                main_percents,
                main_series,
                metric_key="mcc",
                title="LIVE Matthews Correlation Coefficient (MCC)",
                y_label="MCC $\\uparrow$",
                out_file=os.path.join(main_dir, "comparison_mcc_live.png"),
                lower_is_better=False,
            )
...
def _extract_metric_csv_meta(csv_path):
    ...
    mode = "PINN" if "Outputs_mlp_pinn" in csv_path else "NO_PINN"
    train_pct = None
    seed = None
...
def extract_meta_from_path(path):
    ...
    if os.path.abspath(path).startswith(os.path.abspath(OUTPUT_PINN_ROOT) + os.sep):
        config = 'PINN'
    elif 'Outputs_mlp_baseline' in path:
        config = 'NO_PINN'
    else:
        config = 'UNKNOWN'
...
if __name__ == "__main__":
    ...
    print(f"[CONFIG] Warmup values: {', '.join(str(w) for w in WARMUP_EPOCHS)}")
    ...
    baseline_warmup = int(os.environ.get("BASELINE_WARMUP", "0"))
    ...
    # Allow alpha==0 as regular regression (No-PINN mode)
    # if 0 in alpha_test_values:
    #     alpha_test_values = [a for a in alpha_test_values if a != 0]
    ...
    # 3) WARMUP TEST: PINN only, fixed alpha
    warmup_test_alpha = float(os.environ.get("WARMUP_TEST_ALPHA", f"{best_alpha:.12g}"))
    warmup_test_values = _parse_int_list(
        os.environ.get("WARMUP_TEST_VALUES", ",".join(str(w) for w in WARMUP_EPOCHS)),
        "WARMUP_TEST_VALUES",
        min_value=0,
    )
    warmup_test_percents = _parse_int_list(
        os.environ.get("WARMUP_TEST_PERCENTS", DEFAULT_PERCENT_LIST),
        "WARMUP_TEST_PERCENTS",
        min_value=1,
        max_value=100,
    )
    warmup_test_seeds = _parse_seed_list(os.environ.get("WARMUP_TEST_SEEDS", "42"))
    ...
    print(f"[CONFIG][warmup] alpha={alpha_to_label(warmup_test_alpha)} values={warmup_test_values} percents={warmup_test_percents} seeds={warmup_test_seeds}")
    ...
    # Warmup test: PINN only, one percent and one seed.
    if "warmup" in test_groups:
        for idx, warmup in enumerate(warmup_test_values, start=1):
            warmup_tag = f"PINN_warmup_a{alpha_to_tag(warmup_test_alpha)}_W{warmup}_E{EPOCHS}"
            if (
                warmup == alpha_test_warmup
                and any(abs(warmup_test_alpha - alpha) < 1e-12 for alpha in alpha_test_values)
            ):
                # Reuse the equivalent alpha-sweep checkpoint instead of
                # training the same alpha/warmup/seed configuration twice.
                warmup_tag = (
                    f"PINN_alpha_a{alpha_to_tag(warmup_test_alpha)}"
                    f"_W{warmup}_E{EPOCHS}"
                )
            series.append(
                make_series_item(
                    key=f"warmup_test_w{warmup}",
                    label=f"PINN a={alpha_to_label(warmup_test_alpha)} W={warmup}",
                    short_label=f"W={warmup}",
                    mode="pinn",
                    script=SCRIPT_PINN,
                    tag=warmup_tag,
                    env={
                        ...
                        "ALPHA_INIT": f"{warmup_test_alpha:.12g}",
                        "SERIES_LABEL": f"PINN alpha={alpha_to_label(warmup_test_alpha)} W={warmup}",
                    },
                    color=palette[(idx - 1) % len(palette)],
                    linestyle="-",
                    marker="s",
                    percent_list=warmup_test_percents,
                    seed_list=warmup_test_seeds,
                    alpha=warmup_test_alpha,
                    warmup=warmup,
                    group="warmup",
                )
            )
    ...
    header = "Pct | " + " | ".join([f"RMSE {item['short_label']} | MAE {item['short_label']} | NRMSE {item['short_label']} | ACC {item['short_label']}" for item in series])
    print(header)
    for p in target_percents:
        ...
        for item in series:
            ...
            if rec is None:
                cells.extend(["nan", "nan", "nan", "nan"])
            else:
                rmse_str = f"{rec.get('rmse', np.nan):.4f}"
                mae_str = f"{rec.get('mae', np.nan):.4f}"
                nrmse_str = f"{rec.get('nrmse', np.nan):.2f}%"
                acc_str = f"{rec.get('acc', np.nan):.2f}%"
                cells.extend([rmse_str, mae_str, nrmse_str, acc_str])
        print(" | ".join(cells))

    # ---- 3. PLOTTING (GLOBAL LINES, SPLIT SEED TEST) ----
    main_series = [s for s in series if s.get("group") != "seed"]
    ...
    max_error_plot = os.path.join(REPORT_DIR, "comparison_max_error_main_groups.png")
    ...
    balanced_acc_plot = os.path.join(REPORT_DIR, "comparison_balanced_acc_main_groups.png")
    f1_macro_plot = os.path.join(REPORT_DIR, "comparison_f1_macro_main_groups.png")
    mcc_plot = os.path.join(REPORT_DIR, "comparison_mcc_main_groups.png")
    ...
    if main_series:
        ...
        if main_percents:
            save_line_plot(
                ...
                main_series,
                metric_key="mae",
                title="Average Output Test MAE - main groups",
                y_label="Average Test MAE (mm) $\\downarrow$",
                out_file=mae_plot,
                lower_is_better=True,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="rmse",
                title="Average Output Test RMSE - main groups",
                y_label="Average Test RMSE (mm) $\\downarrow$",
                out_file=rmse_plot,
                lower_is_better=True,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="r2",
                title="Average Output Test R² Score - main groups",
                y_label="Average Test R² Score $\\uparrow$",
                out_file=r2_plot,
                lower_is_better=False,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="max_error",
                title="Output Test Max Error - main groups",
                y_label="Max Error (mm) $\\downarrow$",
                out_file=max_error_plot,
                lower_is_better=True,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="nrmse",
                title="Average Output Test NRMSE - main groups",
                y_label="Average Test NRMSE (%) $\\downarrow$",
                out_file=nrmse_plot,
                lower_is_better=True,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="acc",
                title="Classification Accuracy - main groups",
                y_label="Classification Accuracy (%) $\\uparrow$",
                out_file=acc_plot,
                lower_is_better=False,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="balanced_acc",
                title="Balanced Classification Accuracy - main groups",
                y_label="Balanced Accuracy (%) $\\uparrow$",
                out_file=balanced_acc_plot,
                lower_is_better=False,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="f1_macro",
                title="Classification F1-Macro - main groups",
                y_label="F1-Macro (%) $\\uparrow$",
                out_file=f1_macro_plot,
                lower_is_better=False,
            )
            ...
            save_line_plot(
                ...
                main_series,
                metric_key="mcc",
                title="Matthews Correlation Coefficient (MCC) - main groups",
                y_label="MCC $\\uparrow$",
                out_file=mcc_plot,
                lower_is_better=False,
            )
        ...
        if per_shape_csvs:
            ...
            for csv in per_shape_csvs:
                try:
                    ...
                    mode = "PINN" if "Outputs_mlp_pinn" in csv else "NO_PINN"
                    train_pct = None
                    seed = None
    ...
    print(f"[OK] Global R² plot:   {r2_plot}")
