...
def _log_uncaught_exception(exc_type, exc_value, exc_tb):
    script_name = os.path.basename(__file__) if '__file__' in globals() else 'run_eval_xiong.py'
...
SCRIPT_NO_PINN = os.path.join(SCRIPT_DIR, "xiong_2023_baseline.py")
SCRIPT_PINN = os.path.join(SCRIPT_DIR, "xiong_2023_baseline.py")
...
OUTPUT_ROOT = os.path.abspath(
    os.environ.get("EVAL_OUTPUT_ROOT", os.path.join(SCRIPT_DIR, "Outputs_xiong_eval"))
)
STUDY_ROOT = os.path.join(OUTPUT_ROOT, STUDY_NAME)
PINN_MODEL_ROOT = os.path.join(SCRIPT_DIR, "Outputs_xiong_pinn")
NO_PINN_MODEL_ROOT = os.path.join(SCRIPT_DIR, "Outputs_xiong_baseline")
...
def save_single_model_feature_visualizations(
    ...
    for feature_key, feature_label, raw_features in [
        ("backbone", "Shared Xiong Baseline Backbone", backbone_features),
...
def _reeval_infer_existing_model(run_dir, mode, percent, seed, item):
    ...
    if 'y_pred_shape' in locals() and y_pred_shape is not None:
    ...
    config_txt = "\n".join([
        ...
        "",
        "RESULTS (REGRESSION - Q1 STANDARDS)",
        "=" * 80,
    ...
    print(
        ...
        f"rmse={result_row['rmse_avg']:.4f} mae={result_row['mae_avg']:.4f} nrmse={result_row['nrmse_avg']:.2f}%"
    )
    return result_row
...
def _missing_metric_groups(run_dir, item, percent, seed):
    ...
    regression_shape = {
        ...
        for suffix in ("w", "l", "d")
    } | {"mae_avg", "rmse_avg", "r2_avg", "nrmse_avg", "max_error_avg", "bias_avg", "n_samples"}
    ...
    # feature_space is optional visualization, do not block or force reeval
    return missing
...
def save_all_metrics_long_csv(series_items, report_dir):
    ...
    classification_overall_metrics = {
        ...
        "recall_weighted", "mcc", "cohen_kappa", "n_samples",
    }
            ...
            try:
                ...
                if len(df) > 0:
                    ...
                    for col, val in row.items():
                        if col in {"mae_avg", "rmse_avg", "r2_avg", "nrmse_avg", "max_error_avg"}:
                            continue
                        try:
                            rec[col] = float(val)
                        except Exception:
...
def save_line_plot(tgt_pcts, series_dict, metric_key, title, y_label, out_file, lower_is_better):
    ...
    for s_idx, item in enumerate(plot_series):
        ...
        for p, v in valid_pts:
            if metric_key == "nrmse" or "%" in y_label:
                val_text = f"{v:.2f}%"
            else:
...
def _extract_metric_csv_meta(csv_path):
    ...
    mode = "PINN" if "Outputs_xiong_pinn" in csv_path else "NO_PINN"
    train_pct = None
    seed = None
...
def extract_meta_from_path(path):
    ...
    if os.path.abspath(path).startswith(os.path.abspath(OUTPUT_PINN_ROOT) + os.sep):
        config = 'PINN'
    elif 'Outputs_xiong_baseline' in path:
        config = 'NO_PINN'
    else:
        config = 'UNKNOWN'
...
if __name__ == "__main__":
            ...
            for seed in seed_list:
                ...
                if rec is not None:
                    for key, value in rec.items():
                        if key in {"rmse", "mae", "nrmse", "r2", "max_error", "run_dir", "csv_file"}:
                            continue
                        row[key] = value
                rows.append(row)
    ...
    stats_rows = []
    stat_metrics = [("rmse", "RMSE (mm)", "rmse_avg"), ("mae", "MAE (mm)", "mae_avg"), ("nrmse", "NRMSE (%)", "nrmse_avg")]
    ...
    header = "Pct | " + " | ".join([f"RMSE {item['short_label']} | MAE {item['short_label']}" for item in series])
    print(header)
    for p in target_percents:
        ...
        for item in series:
            ...
            if rec is None:
                ...
                r2_str = f"{rec.get('r2', np.nan):.4f}"
                cells.extend([rmse_str, mae_str, nrmse_str, r2_str])
        print(" | ".join(cells))
        ...
        if per_shape_csvs:
            ...
            for csv in per_shape_csvs:
                try:
                    ...
                    mode = "PINN" if "Outputs_xiong_pinn" in csv else "NO_PINN"
                    train_pct = None
                    seed = None
