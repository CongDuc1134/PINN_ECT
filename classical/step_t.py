...
# ... (Giữ nguyên các hàm calSR1 -> calSR6, InteT_add3, generic_integration từ code trước) ...
...
def calTST(X, Y, z, lc, dc, delta, xx, zz):
    y_term = (Y + lc / 3 / dc * zz + lc / 3) ** 2
    return torch.exp(zz / delta) * (z - zz) / ((X - xx) ** 2 + y_term + (z - zz) ** 2) ** 1.5
...
def InteT_add3_gpu(X, Y, z, wc, lc, dc, delta, m=20, n=20):
    a, b = -lc / 2, 0
    x_vec = linspace_with_grad(a, b, m + 1)
    c_vec = -x_vec * 2 * dc / lc - dc
    d_vec = torch.zeros_like(c_vec)
    h_vec = (d_vec - c_vec) / n
    t = torch.linspace(0, 1, n + 1, device=device, dtype=dtype)
    yy = c_vec[:, None] + t[None, :] * h_vec[:, None]
    zz = yy
    YY, ZZ = yy[:, :, None, None], zz[:, :, None, None]
    term = (1 / ((X + wc / 2) ** 2 + (Y - YY) ** 2 + (z - ZZ) ** 2) ** 1.5 +
            1 / ((X - wc / 2) ** 2 + (Y - YY) ** 2 + (z - ZZ) ** 2) ** 1.5)
...
def calculate_field_Step_T(X, Y, z, wc, lc, dc, delta):
    ...
    v_add3 = InteT_add3_gpu(X, Y - lc / 6, z, wc, 2 * lc / 3, dc / 2, delta)
    v_tst = generic_integration(calTST, X, Y, z, {'lc': lc, 'dc': dc, 'delta': delta}, (-wc/2, wc/2), (-dc, 0))
    return v1 + v2 - v5 - v6 + v_add3 + v_tst
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        H_val = calculate_field_Step_T(X_grid, Y_grid, z_lift, wc, lc, dc, delta)
    ...
    ax2.plot(dist_length, section_data, 'b-', linewidth=2)
