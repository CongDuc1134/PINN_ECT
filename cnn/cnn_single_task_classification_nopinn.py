# -*- coding: utf-8 -*-
"""
Single-Task 2D CNN Defect Shape Classification WITHOUT PINN (NoPINN Baseline)
Architecture: 2D CNN Backbone + 5-Class Defect Shape Classification Head
Purely data-driven classification without regression branch and without physics loss.
"""
...
OUTPUT_ROOT_DIR = os.environ.get(
    "OUTPUT_ROOT", os.path.join(SCRIPT_DIR, "Outputs_cnn_single_task_classification_nopinn")
)
...
parser = argparse.ArgumentParser(description="Single-Task 2D CNN Classification NoPINN")
...
# ============================================================================
# MODEL DEFINITION: SINGLE-TASK CLASSIFICATION CNN (NO REGRESSION HEAD)
# ============================================================================

class CNN_SingleTask_Classification(nn.Module):
    """
    2D CNN for 5-class defect shape classification without regression head.
    """
...
def main():
    print(f"\n{'='*80}")
    print(f"SINGLE-TASK 2D CNN CLASSIFICATION WITHOUT PINN")
    ...
    train_loader = DataLoader(TensorDataset(X_tr_t, y_sh_tr_t), batch_size=BATCH_SIZE, shuffle=True)
    ...
    model = CNN_SingleTask_Classification(num_shapes=num_shapes).to(device)
    ...
    print(f"\n{'='*60}")
    print(f"TEST RESULTS (SINGLE-TASK CLASSIFICATION NOPINN):")
    ...
    res_df = pd.DataFrame([{
        'Model': '2D CNN Single-Task Classification (NoPINN)',
