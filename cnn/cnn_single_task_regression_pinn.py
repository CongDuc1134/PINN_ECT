# -*- coding: utf-8 -*-
"""
Single-Task 2D CNN Defect Sizing Regression WITH PINN (Physics-Informed)
Architecture: 2D CNN Backbone + Joint Sizing Regression Head (W, L, D) + DMM Physics Loss
Purely regression (no classification head) regularized by analytical DMM dipole forward solver.
"""
...
OUTPUT_ROOT_DIR = os.environ.get(
    "OUTPUT_ROOT", os.path.join(SCRIPT_DIR, "Outputs_cnn_single_task_regression_pinn")
)
...
parser = argparse.ArgumentParser(description="Single-Task 2D CNN Regression with PINN")
...
# ============================================================================
# MODEL DEFINITION: SINGLE-TASK REGRESSION CNN WITH PINN
# ============================================================================

class CNN_SingleTask_Regression_PINN(nn.Module):
    """
    2D CNN for defect sizing regression (W, L, D) with Kendall et al. Homoscedastic
    Uncertainty Weighting across W, L, D and regularized with analytical DMM physics loss.
    """
    def __init__(self):
        super(CNN_SingleTask_Regression_PINN, self).__init__()
...
def calculate_pinn_sample_loss_torch(w_pred, l_pred, d_pred, shape_name, H_true_tensor,
    ...
    """
    Computes analytical DMM differenced magnetic field loss for a single sample.
    """
...
def main():
    print(f"\n{'='*80}")
    print(f"SINGLE-TASK 2D CNN REGRESSION WITH PINN")
    ...
    # Keep unnormalized field for analytical DMM comparison
    ...
    train_loader = DataLoader(TensorDataset(X_tr_t, y_tr_t, y_sh_tr_t, idx_tr_t), batch_size=BATCH_SIZE, shuffle=True)
    ...
    model = CNN_SingleTask_Regression_PINN().to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for bx, by, b_shape, b_idx in train_loader:
            bx, by = bx.to(device), by.to(device)
            optimizer.zero_grad()
            
            pred = model(bx)
            _, _, _, reg_loss, _ = compute_regression_losses(model, pred, by, criterion_reg)
            ...
            if pinn_active:
                ...
                for i in range(len(bx)):
                    s_idx = b_idx[i].item()
                    sh_name = unique_shapes[b_shape[i].item()]
                    w_p = pred[i, 0] * w_max
                    l_p = pred[i, 1] * l_max
                    d_p = pred[i, 2] * d_max
                    H_true_s = X_train_field[s_idx]
                    p_loss = calculate_pinn_sample_loss_torch(w_p, l_p, d_p, sh_name, H_true_s)
                    batch_pinn_loss = batch_pinn_loss + p_loss
    ...
    print(f"\n{'='*60}")
    print(f"TEST RESULTS (SINGLE-TASK REGRESSION PINN):")
    ...
    res_df = pd.DataFrame([{
        'Model': '2D CNN Single-Task Regression (PINN)',
