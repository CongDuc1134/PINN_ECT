...
try:
    ...
    from tensorflow.keras.layers import Dense, Dropout, Activation, Flatten, Conv2D, MaxPool2D, Conv2DTranspose, MaxPooling2D, concatenate
...
def Load_Data_Test(Path, list_name):
  """
  Nạp dữ liệu thực nghiệm kiểm thử từ danh sách file trong thư mục Path.
  Tự động căn chỉnh ma trận 32x32 và tính toán kênh gradient không gian (∇H),
  trả về danh sách mảng 2 kênh (32, 32, 2).
  """
  X = []
  for i in tqdm(list_name, desc="📊 Loading Test Data", unit="file", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt}'):
    ...
    try:
      with open(filename, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        tam = []
        line = [row for row in reader if row]
        for k in range(len(line)):
          tam1 = [float(val) for val in line[k] if val.strip() != '']
          if len(tam1) > 0:
            tam.append(tam1)

        draft = np.array(tam, dtype=np.float32)
        if draft.shape == (31, 31):
          draft = np.vstack([draft, draft[-1:, :]])
          draft = np.hstack([draft, draft[:, -1:]])
        elif draft.shape[0] == 32 and draft.shape[1] == 31:
          draft = np.hstack([draft, draft[:, -1:]])
        elif draft.shape != (32, 32):
          draft = cv2.resize(draft, (32, 32), interpolation=cv2.INTER_CUBIC)

        grad_y = np.gradient(draft, axis=0)
        grad_x = np.gradient(draft, axis=1)
        grad_mag = np.sqrt(grad_x**2 + grad_y**2)

        two_channel = np.dstack([draft, grad_mag])
        X.append(two_channel)
    except Exception as e:
      print(f"[CẢNH BÁO] Không thể đọc file test {filename}: {e}")
      continue
